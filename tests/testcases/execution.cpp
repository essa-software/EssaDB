...
DbErrorOr<void> select_simple() {
    ...
    auto result = TRY(Db::Sql::run_query(db, "SELECT * FROM test;")).as_result_set();
...
DbErrorOr<void> select_columns() {
    auto db = TRY(setup_db());
    auto result = TRY(Db::Sql::run_query(db, "SELECT number, string FROM test;")).as_result_set();
...
DbErrorOr<void> select_order_by() {
    auto db = TRY(setup_db());
    auto result = TRY(Db::Sql::run_query(db, "SELECT id AS [ID], number AS [NUMBER], string AS [STRING] FROM test ORDER BY string, number DESC;")).as_result_set();
...
DbErrorOr<void> select_order_by_desc() {
    auto db = TRY(setup_db());
    auto result = TRY(Db::Sql::run_query(db, "SELECT id AS [ID], number AS [NUMBER], string AS [STRING] FROM test ORDER BY number DESC;")).as_result_set();
...
DbErrorOr<void> select_top_number() {
    auto db = TRY(setup_db());
    auto result = TRY(Db::Sql::run_query(db, "SELECT TOP 2 * FROM test;")).as_result_set();
...
DbErrorOr<void> select_top_perc() {
    auto db = TRY(setup_db());
    auto result = TRY(Db::Sql::run_query(db, "SELECT TOP 75 PERC * FROM test;")).as_result_set();
...
DbErrorOr<void> select_with_union() {
    auto db = TRY(setup_db());
    auto result = TRY(Db::Sql::run_query(db, "SELECT id AS ID, number AS NUM, string AS STR FROM test ORDER BY number ASC UNION SELECT id AS ID, number AS NUM, string AS STR FROM test ORDER BY number DESC")).as_result_set();
...
DbErrorOr<void> select_with_union_all() {
    auto db = TRY(setup_db());
    auto result = TRY(Db::Sql::run_query(db, "SELECT id AS ID, number AS NUM, string AS STR FROM test ORDER BY number ASC UNION ALL SELECT id AS ID, number AS NUM, string AS STR FROM test ORDER BY number DESC")).as_result_set();
    TRY(expect(result.column_names() == std::vector<std::string> { "ID", "NUM", "STR" }, "columns have alias names"));
...
DbErrorOr<void> select_with_aliases() {
    auto db = TRY(setup_db());
    auto result = TRY(Db::Sql::run_query(db, "SELECT id AS ID, number AS NUM, string AS STR FROM test;")).as_result_set();
    TRY(expect(result.column_names() == std::vector<std::string> { "ID", "NUM", "STR" }, "columns have alias names"));
    TRY(expect_equal<size_t>(result.rows().size(), 7, "all rows were returned"));
    return {};
}
...
DbErrorOr<void> select_aliases_with_square_brackets() {
    auto db = TRY(setup_db());
    auto result = TRY(Db::Sql::run_query(db, "SELECT id AS [id], number AS [some number], string AS [ some  string ] FROM test;")).as_result_set();
...
DbErrorOr<void> select_case() {
    auto db = TRY(setup_db());
    auto result = TRY(Db::Sql::run_query(db, "SELECT id AS [ID], CASE WHEN id < 2 THEN 'less than 2' WHEN id < 4 THEN 'less than 4' ELSE 'other cases' END AS [CASE RESULT] FROM test;")).as_result_set();
    // TRY(expect(result.column_names()[0] == "id", "square brackets are parsed properly"));
    return {};
}
