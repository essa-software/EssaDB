...
DbErrorOr<void> csv_export_import() {
    ...
    auto* new_table = &db.create_table("newtest", nullptr, std::map<std::string, std::shared_ptr<AST::Expression>> {});
...
DbErrorOr<void> csv_export_import_with_aliases() {
    ...
    auto& new_table = db.create_table("new_test", nullptr, std::map<std::string, std::shared_ptr<AST::Expression>> {});
    TRY(new_table.import_from_csv("test.csv"));
